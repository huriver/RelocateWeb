...
public class DriverTruckTypeServiceImpl implements DriverTruckTypeService {
    ...
    public PageResult pageQuery(DriverTruckTypePageQueryDTO pageQueryDTO) {
        ...
        Page<DriverTruckTypeVO> page = driverTruckTypeMapper.pageQuery(pageQueryDTO);
...
//        // 司机能驾驭的每个货车类型下只能有1辆车
//        // 新增业务校验：司机在该货车类型下是否已存在司机的可驾驶货车类型
...
//            log.error("新增司机的可驾驶货车类型失败：司机 ID {} 在货车类型 ID {} 下已存在司机的可驾驶货车类型", vehicleDTO.getDriverId(), vehicleDTO.getTruckTypeId());
...
//        if (count > 0) {
//            log.error("新增司机的可驾驶货车类型失败：车牌号已存在: {}", vehicleDTO.getLicensePlateNumber());
...
//        );
//        if (vehicleCount != null && vehicleCount > 0) {
//            log.error("修改司机的可驾驶货车类型失败：修改后的司机 ID {} 在货车类型 ID {} 下已存在**其他**司机的可驾驶货车类型",
//                    vehicleDTO.getDriverId(),
//                    vehicleDTO.getTruckTypeId());
//            throw new BusinessException(MessageConstant.DRIVER_TRUCK_TYPE_VEHICLE_EXIST);
//        }
//
//        // ====== 业务校验：修改后的车牌号是否与**其他**司机的可驾驶货车类型重复 ======
//        Integer duplicateCount = vehicleMapper.countByLicensePlateNumberExcludeId(
//                vehicleDTO.getLicensePlateNumber(),
//                vehicleDTO.getId()
//        );
//        if (duplicateCount != null && duplicateCount > 0) {
//            log.error("修改司机的可驾驶货车类型失败：车牌号已存在于其他司机的可驾驶货车类型: {}", vehicleDTO.getLicensePlateNumber());
//            throw new BusinessException(MessageConstant.LICENSE_PLATE_NUMBER_EXIST);
//        }
//
//        // 将 DTO 对象属性拷贝到实体类对象
...
//        // 业务校验：检查当前司机的可驾驶货车类型是否关联了未完成订单
//        Integer orderCount = orderMapper.countByAssignedVehicleId(id);
//        if (orderCount != null && orderCount > 0) {
//            log.error("司机的可驾驶货车类型删除失败：ID {} 关联了 {} 个未完成订单", id, orderCount);
