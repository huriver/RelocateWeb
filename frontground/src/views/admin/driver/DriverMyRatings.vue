...
<script setup>
  // D:\Java\code\RelocateWeb\frontground\src\views\admin\driver\DriverMyRatings.vue
  ...
  // 导入司机评价相关的 API
  import {
    getDriverMyRatingsListApi,
    getDriverMyRatingDetailApi,
  ...
  const fetchMyRatingsList = async () => {
    ...
    try {
      ...
      const { data: res } = await getDriverMyRatingsListApi(params); // 调用司机我的评价列表 API
      ...
      if (res.code === 1) {
        ...
        console.warn('获取司机我的评价列表业务失败:', res.msg);
      }
    } catch (error) {
      // 请求本身发生错误 (网络问题, HTTP错误等)，消息已由拦截器弹出
      console.error('获取司机我的评价列表请求失败:', error);
      tableData.value = [];
      pagination.value.total = 0;
    } finally {
      loading.value = false;
  ...
  const showDetails = async (row) => {
    ...
    try {
      // 调用司机我的评价详情 API 根据ID获取完整的详情数据
      const { data: res } = await getDriverMyRatingDetailApi(row.id); // 使用 row.id (评价ID)
      ...
      if (res.code === 1 && res.data) {
        ...
        console.warn('获取司机我的评价详情业务失败:', res.msg);
      }
    } catch (error) {
      // 请求本身发生错误，消息已由拦截器弹出，这里只需关闭弹窗
      console.error('获取司机我的评价详情请求失败:', error);
      detailDialogVisible.value = false;
    }
  };
