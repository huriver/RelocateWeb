<script setup>
  // D:\Java\code\RelocateWeb\frontground\src\views\admin\driver\orders\DriverPendingOrders.vue
  ...
  import {
    ...
    ElTableColumn,
    ElPagination,
    ElInputNumber, // 可能用于价格相关字段显示，虽然API详情里有，列表没有
  ...
  import {
    ...
    getDriverAvailableOrderDetailApi,
    acceptDriverOrderApi,
  } from '@/api/orderApi.js'; // 确保路径正确

  // 导入司机与车型关联的 API
  import { getBackDriverTruckTypeDetailApi } from '@/api/driverTruckTypeApi.js'; // 假设此API返回司机关联的车型列表
  ...
  // 获取 store 实例和当前司机 ID
  const store = myStore();
  // 使用 computed 获取当前登录司机的信息和 ID
  const currentDriverInfo = computed(() => store.backUserInfo);
  const currentDriverId = computed(() => currentDriverInfo.value ? currentDriverInfo.value.id : null);
  ...
  // 司机可驾驭的车型列表 (用于筛选下拉框)
  const driverTruckTypes = ref([]);

  // 服务类别列表 (用于筛选下拉框)
  const serviceCategories = ref([]); // 新增：存储服务类别列表

  // 搜索表单数据
  const searchForm = ref({
    reservationTimeRange: null, // 预约时间范围 [start, end]
    truckTypeId: null, // 所需车型 ID
    serviceCategoryId: null, // 新增：服务类别 ID
  });
    ...
    try {
      ...
      const params = {
        ...
        endDate: searchForm.value.reservationTimeRange
          ...
          : undefined,
        // 包含所需车型 ID 筛选条件
        truckTypeId: searchForm.value.truckTypeId || undefined, // 如果为 null 或 undefined 则不传递该参数
        // 包含服务类别 ID 筛选条件
        serviceCategoryId: searchForm.value.serviceCategoryId || undefined, // 新增：如果为 null 或 undefined 则不传递该参数
      };

      const { data: res } = await getDriverAvailableOrdersApi(params); // 调用司机待接订单列表 API
      ...
      if (res.code === 1) {
        ...
        console.warn('获取司机待接订单列表业务失败:', res.msg);
      }
    } catch (error) {
      // 请求本身发生错误 (网络问题, HTTP错误等)，消息已由拦截器弹出
      console.error('获取司机待接订单列表请求失败:', error);
      tableData.value = [];
      pagination.value.total = 0;
    } finally {
      loading.value = false;
  ...
  // === 获取司机关联车型列表 (用于筛选下拉框) ===
  const fetchDriverTruckTypes = async () => {
    // 确保已获取到当前司机 ID
    if (!currentDriverId.value) {
      console.warn("当前司机 ID 未获取到，无法加载可驾驭车型列表。");
      // 如果没有司机ID，则清空车型列表
      driverTruckTypes.value = [];
      return;
    }
    try {
      // 调用 API 获取当前司机关联的车型列表
      const { data: res } = await getBackDriverTruckTypeDetailApi(currentDriverId.value);

      // request.js 拦截器处理错误提示
      // 确保 code 为 1 且 data 存在，并且 data.truckTypeSimpleVOList 是一个数组
      if (res.code === 1 && res.data && Array.isArray(res.data.truckTypeSimpleVOList)) {
        // 从 res.data.truckTypeSimpleVOList 中获取车型列表
        driverTruckTypes.value = res.data.truckTypeSimpleVOList.map(type => ({
          value: type.truckTypeId, // 使用 truckTypeId 作为 value
          label: type.truckTypeName // 使用 truckTypeName 作为 label
        }));
      } else {
        console.warn('获取司机可驾驭车型列表业务失败或数据格式错误:', res.msg, res.data);
        // 失败时清空车型列表
        driverTruckTypes.value = [];
      }
    } catch (error) {
      console.error('获取司机可驾驭车型列表请求失败:', error);
      // 请求失败时清空车型列表
      driverTruckTypes.value = [];
    }
  };
  ...
  const resetSearchForm = () => {
    searchForm.value = {
      ...
      truckTypeId: null, // 重置所需车型 ID
      serviceCategoryId: null, // 新增：重置服务类别 ID
    };
  ...
  const showDetails = async (row) => {
    ...
    try {
      // 调用司机待接订单详情 API 根据ID获取完整的详情数据
      const { data: res } = await getDriverAvailableOrderDetailApi(row.orderId); // 使用 row.orderId
      ...
      if (res.code === 1 && res.data) {
        ...
        console.warn('获取司机待接订单详情业务失败:', res.msg);
      }
    } catch (error) {
      // 请求本身发生错误，消息已由拦截器弹出，这里只需关闭弹窗
      console.error('获取司机待接订单详情请求失败:', error);
      detailDialogVisible.value = false;
    }
  };
  ...
  const handleAcceptOrder = (row) => {
    ElMessageBox.confirm(`确定要接取订单号为《${row.orderNumber}》的订单吗？`, '接单确认', {
      ...
      .then(async () => {
        try {
          // 调用接单 API
          const { data: res } = await acceptDriverOrderApi({ orderId: row.orderId }); // 发送包含 orderId 的请求体
  ...
  onMounted(() => {
    ...
    fetchDriverTruckTypes(); // Fetch driver's associated truck types for truck type filter
    fetchServiceCategories(); // 新增：Fetch service categories for service category filter
  });
</script>
  ...
  <div class="page-container">
    ...
    <el-form :inline="true" :model="searchForm" class="search-form">
      <div class="input-items-group">
        ...
        <el-form-item label="所需车型">
          <el-select v-model="searchForm.truckTypeId" placeholder="请选择车型" clearable
                     style="width: 150px;">
            <el-option v-for="item in driverTruckTypes" :key="item.value" :label="item.label"
    ...
    <el-table :data="tableData" v-loading="loading" border stripe style="width: 100%">
      ...
      <el-table-column prop="truckTypeName" label="所需车型" align="center" width="170"
                       show-overflow-tooltip></el-table-column>
      <el-table-column prop="numberOfHelpers" label="工人数量" align="center"
...
<style scoped lang="less">
  /* 直接复用 AdminServiceTypeConfig.vue 的样式 */
  .page-container {
    padding: 20px; // 整体内边距
