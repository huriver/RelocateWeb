<script setup>
  // D:\Java\code\RelocateWeb\frontground\src\views\admin\driver\orders\DriverHistoryOrders.vue
  ...
  // 导入司机历史订单相关的 API
  import {
    getDriverHistoricalOrdersListApi,
    getDriverHistoricalOrderDetailApi,
    // 导入司机/搬家工人历史订单专属状态接口
  ...
  const fetchHistoricalOrdersList = async () => {
    ...
    try {
      ...
      // 注意：这里的 beginEndTime 和 endEndTime 在 API 文档示例中是日期时间格式
      // ElDatePicker 结合 value-format="YYYY-MM-DD HH:mm:ss" 可以处理
      // 但对于日期范围，通常 ElDatePicker 默认 value-format="YYYY-MM-DD"，然后后端会处理当天的时间范围
      // 如果后端严格要求时间，需要调整 value-format 并设置 default-time
      // 暂且使用YYYY-MM-DD 格式，并依赖后端处理全天范围
      ...
      const { data: res } = await getDriverHistoricalOrdersListApi(params); // 调用司机历史订单列表 API
      ...
      if (res.code === 1) {
        ...
        console.warn('获取司机历史订单列表业务失败:', res.msg);
      }
    } catch (error) {
      // 请求本身发生错误 (网络问题, HTTP错误等)，消息已由拦截器弹出
      console.error('获取司机历史订单列表请求失败:', error);
      tableData.value = [];
      pagination.value.total = 0;
    } finally {
      loading.value = false;
  ...
  // === 辅助方法：获取订单状态对应的 Tag 类型 (从 DriverMyOrders.vue 复制，适配历史订单状态) ===
  const getOrderStatusTagType = (status) => {
    switch (status) {
      ...
      case 5:
        return 'danger'; // 已取消
      // 其他状态可能不会出现在历史订单中，但为了兼容性可以保留或设置为默认
      case 0:
  ...
  const resetSearchForm = () => {
    ...
    // 重置时不再执行查询，仅清空表单，用户需要手动点击查询
    // 如果需要重置后立即查询所有，可以调用 handleSearch()
    handleSearch(); // 根据需要选择是否在重置时自动查询
  };
  ...
  const showDetails = async (row) => {
    ...
    try {
      // 调用司机历史订单详情 API 根据ID获取完整的详情数据
      const { data: res } = await getDriverHistoricalOrderDetailApi(row.orderId); // 使用 row.orderId
      ...
      if (res.code === 1 && res.data) {
        ...
        console.warn('获取司机历史订单详情业务失败:', res.msg);
      }
    } catch (error) {
      // 请求本身发生错误，消息已由拦截器弹出，这里只需关闭弹窗
      console.error('获取司机历史订单详情请求失败:', error);
      detailDialogVisible.value = false;
    }
  };
  ...
  <div class="page-container">
    ...
    <el-table :data="tableData" v-loading="loading" border stripe style="width: 100%">
      ...
      <el-table-column prop="customerName" label="客户姓名" align="center"
                       width="120"></el-table-column> <el-table-column prop="movingPrice"
                       label="实际费用" align="center" width="120"></el-table-column> <el-table-column
