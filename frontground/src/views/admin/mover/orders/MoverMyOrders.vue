<script setup>
  // D:\Java\code\RelocateWeb\frontground\src\views\admin\mover\orders\MoverMyOrders.vue
  ...
  // 导入搬家工人我的订单相关的 API 以及共享状态接口
  import {
    getMoverMyOrdersListApi, // 搬家工人我的订单列表 API
    getMoverMyOrderDetailApi, // 搬家工人我的订单详情 API
    cancelMoverOrderApi, // 搬家工人取消订单 API
    getDriverMoverMyOrdersStatusesApi, // 司机/搬家工人我的订单专属状态接口
  ...
  const fetchMyOrdersList = async () => {
    ...
    try {
      ...
      // 调用搬家工人我的订单列表 API
      const { data: res } = await getMoverMyOrdersListApi(params);
      ...
      if (res.code === 1) {
        ...
        console.warn('获取搬家工人我的订单列表业务失败:', res.msg);
      }
    } catch (error) {
      // 请求本身发生错误 (网络问题, HTTP错误等)，消息已由拦截器弹出
      console.error('获取搬家工人我的订单列表请求失败:', error);
      tableData.value = [];
      pagination.value.total = 0;
    } finally {
      loading.value = false;
  ...
  // === 辅助方法：获取订单状态对应的 Tag 类型 (从 DriverMyOrders.vue 复制) ===
  // 注意：这里假设搬家工人订单状态的code和司机订单状态的code对应相同 Tag 类型
  const getOrderStatusTagType = (status) => {
    switch (status) {
      case 0:
        return 'info'; // 待接单 (此状态通常不会出现在搬家工人我的订单中，但保留兼容性)
      case 1:
        return ''; // 司机已接单，等待搬运工人 (默认颜色)
      case 2:
        return 'primary'; // 已接单（团队已确认）
      case 3:
        return 'warning'; // 进行中
      case 4:
        return 'success'; // 已完成 (此状态通常不会出现在搬家工人我的订单中，但保留兼容性)
      case 5:
        return 'danger'; // 已取消 (此状态通常不会出现在搬家工人我的订单中，但保留兼容性)
      default:
        return 'info';
    }
  };
  ...
  const showDetails = async (row) => {
    ...
    try {
      // 调用搬家工人我的订单详情 API 根据ID获取完整的详情数据
      const { data: res } = await getMoverMyOrderDetailApi(row.orderId); // 使用 row.orderId
      ...
      if (res.code === 1 && res.data) {
        ...
        console.warn('获取搬家工人我的订单详情业务失败:', res.msg);
      }
    } catch (error) {
      // 请求本身发生错误，消息已由拦截器弹出，这里只需关闭弹窗
      console.error('获取搬家工人我的订单详情请求失败:', error);
      detailDialogVisible.value = false;
    }
  };
  ...
  const submitCancel = async () => {
    ...
    try {
      // 调用搬家工人取消订单 API
      const { data: res } = await cancelMoverOrderApi({
  ...
  // === 开始搬运服务相关方法 & 完成搬运服务相关方法 ===
  // 注意：根据您提供的 API，没有搬家工人单独开始/完成订单的接口，这些操作通常由司机执行。
  // 因此，这里移除了司机MyOrders中的 handleStartService 和 handleCompleteService 方法以及对应的操作按钮。
  ...
  <div class="page-container">
    ...
    <el-table :data="tableData" v-loading="loading" border stripe style="width: 100%">
      ...
      <el-table-column prop="numberOfHelpers" label="工人数量" align="center" width="120"
                       show-overflow-tooltip></el-table-column>
      <el-table-column prop="driverName" label="司机姓名" align="center" width="120"></el-table-column>
      <el-table-column prop="vehiclePlateNumber" label="车牌号" align="center"
      ...
      <el-table-column label="操作" width="180" align="center" fixed="right">
        <template #default="scope">
          ...
          <el-button v-if="scope.row.orderStatus === 1 || scope.row.orderStatus === 2" size="small"
                     type="danger" style="margin-left: 10px"
                     @click="openCancelDialog(scope.row)">取消订单</el-button>
    ...
    <el-dialog v-model="detailDialogVisible" title="订单详情" width="600px">
      <el-form label-width="150px" v-if="currentRowDetails">
        ...
        <div class="detail-group">
          ...
          <el-form-item label="里程费用">{{ currentRowDetails.mileageCost || 'N/A' }}</el-form-item>
          <el-form-item label="工人费用">{{ currentRowDetails.helperCost || 'N/A' }}</el-form-item>
          <el-form-item
                        label="服务类型价格乘数">{{ currentRowDetails.categoryPriceMultiplier || 'N/A' }}</el-form-item>
        </div>
