<template>
  <div class="my-ratings-container">
    ...
    <div v-else class="ratings-list">
      <el-card v-for="orderRatings in groupedRatingsList" :key="orderRatings.orderNumber"
        ...
        <div class="order-rating-body">
          <div class="entity-ratings">
            <div v-for="entityRating in orderRatings.entityRatings"
              ...
              <div class="entity-info">
                ...
                <el-rate v-model="entityRating.ratingValue" disabled show-score text-color="#ff9900"
...
<script setup>
  ...
  const fetchHistoryRatings = async () => {
    ...
    try {
      ...
      // 检查后端业务状态码 (request.js 确保 HTTP 状态码 200 时 response.data 存在)
      if (response.data && response.data.code === 1) {
        ...
        // ElMessage.error(response.data?.msg || '获取历史评价失败'); // <-- 移除此行，避免重复提示
        console.warn('获取历史评价业务失败:', response.data?.msg); // 可以保留日志
        groupedRatingsList.value = []; // 业务失败时清空列表
      }
    } catch (error) {
      // 捕获真正的请求错误 (网络问题, CORS, request.js 拦截器抛出错误等)
      console.error('获取历史评价请求出错:', error);
      ElMessage.error('获取历史评价失败，请稍后再试'); // <-- 这个用于网络或HTTP错误
      groupedRatingsList.value = []; // 请求出错时清空列表
    } finally {
