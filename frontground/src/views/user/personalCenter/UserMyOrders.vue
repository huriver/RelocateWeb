<template>
  <div class="my-orders-container">
    ...
    <div class="filter-section">
      <el-select v-model="orderFilterPagination.orderStatus" placeholder="筛选订单状态" clearable
                 size="default" style="width: 200px" @change="handleStatusChange">
        <el-option label="全部状态" value=""></el-option>
        <el-option v-for="status in orderStatusOptions" :key="status.code"
                   :label="status.description" :value="status.code">
        </el-option>
      </el-select>
    </div>

    <el-table v-loading="isLoading" :data="orders" style="width: 100%" border stripe
              empty-text="暂无订单数据" header-align="center" class="order-table">
      ...
      <el-table-column prop="movingOrigin" label="起始地点" min-width="200" align="center"
                       show-overflow-tooltip />
      <el-table-column prop="movingDestination" label="目的地点" min-width="200" align="center"
                       show-overflow-tooltip />
      ...
      <el-table-column label="操作" width="250" fixed="right" align="center">
        <template #default="{ row }">
          ...
          <el-button v-if="canPayOrder(row)" link type="success"
                     @click="handlePayOrder(row.id, row.orderNumber)">
            去支付
          </el-button>
          <el-button v-if="canCancelOrder(row.orderStatus)" link type="danger"
    ...
    <div class="pagination-footer">
      <el-pagination v-model:current-page="orderFilterPagination.page"
                     v-model:page-size="orderFilterPagination.pageSize"
                     :page-sizes="[5, 10, 15, 20]" layout="total, sizes, prev, pager, next, jumper"
                     :total="orderFilterPagination.total" @size-change="handleSizeChange"
                     @current-change="handlePageChange" />
    </div>

    <el-dialog v-model="showDetailDialog" title="订单详情" width="60%" class="order-detail-dialog"
      ...
      <div v-else-if="currentOrderDetail">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item
                                label="工人数量">{{ currentOrderDetail.numberOfHelpers }}人</el-descriptions-item>
          <el-descriptions-item
                                label="里程费用">{{ currentOrderDetail.mileageCost.toFixed(2) }}元</el-descriptions-item>
          <el-descriptions-item
                                label="工人费用">{{ currentOrderDetail.helperCost.toFixed(2) }}元</el-descriptions-item>
          <el-descriptions-item label="服务类型价格乘数">{{
      ...
      <template #footer>
        <span class="dialog-footer">
          <el-button v-if="currentOrderDetail && canPayOrder(currentOrderDetail)" link
                     type="success"
                     @click="handlePayOrder(currentOrderDetail.id, currentOrderDetail.orderNumber)">
            去支付
          </el-button>
          <el-button v-if="currentOrderDetail && canCancelOrder(currentOrderDetail.orderStatus)"
                     link type="danger" @click="showCancelOrderDialog(currentOrderDetail.id)">
            取消订单
          </el-button>
          <el-button v-if="currentOrderDetail && canReviewOrder(currentOrderDetail)" link
                     type="primary" @click="handleReviewOrder(currentOrderDetail.id)">
            立即评价
          </el-button>
    ...
    <el-dialog v-model="showCancelDialog" title="取消订单" width="400px" class="cancel-order-dialog"
               :before-close="handleCancelDialogClose">
      <el-form>
        <el-form-item>
          <el-input v-model="cancelReason" type="textarea" :rows="4" placeholder="请输入取消原因（选填）"
                    maxlength="200" show-word-limit class="cancel-reason-input"></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
        <div class="dialog-footer">
          ...
          <el-button type="danger" @click="confirmCancelOrder" :loading="isCancelling"
                     class="confirm-btn">
            确认取消
          </el-button>
        </div>
...
<script setup>
  ...
  const fetchOrderStatusOptions = async () => {
    try {
      const { data: res } = await getFrontOrderStatusApi();
      if (res.code === 1) {
      ...
      ElMessage.error('获取订单状态失败，请检查网络。'); // <-- 这个用于网络或HTTP错误
    }
  };
  ...
  const queryOrders = async () => {
    ...
    try {
      ...
      if (res.code === 1) {
        ...
        // 业务失败 (code !== 1)，request.js 已经弹窗提示了后端 msg
        // ElMessage.error(res.msg || '查询订单失败！'); // <-- 移除此行
        console.warn('查询订单业务失败:', res.msg); // 可以保留日志
        orders.value = [];
      ...
      ElMessage.error('查询订单失败，请检查网络或稍后再试。'); // <-- 这个用于网络或HTTP错误
      orders.value = [];
      orderFilterPagination.value.total = 0;
    } finally {
  ...
  const viewOrderDetail = async (orderId) => {
    ...
    try {
      ...
      if (res.code === 1) {
        ...
        // ====== 添加这两行 DEBUG 代码 ======
        // console.log('获取到的订单详情数据:', currentOrderDetail.value); // 调试时可以打开
        // console.log('尝试获取的订单ID (currentOrderDetail.id):', currentOrderDetail.value.id); // 调试时可以打开
        // ================================
      } else {
        // 业务失败 (code !== 1)，request.js 已经弹窗提示了后端 msg
      ...
      ElMessage.error('获取订单详情失败，请检查网络或稍后再试。'); // <-- 这个用于网络或HTTP错误
    } finally {
  ...
  const confirmCancelOrder = async () => {
    ...
    try {
      ...
      const { data: res } = await cancelFrontOrderApi(currentOrderIdToCancel.value, {
        cancelReason: reason,
      });
      ...
      if (res.code === 1) {
        ...
        if (
          ...
          currentOrderDetail.value.id === currentOrderIdToCancel.value
        ) {
          viewOrderDetail(currentOrderIdToCancel.value); // 刷新详情数据
        }
      } else {
        // 业务失败 (code !== 1)，request.js 已经弹窗提示了后端 msg
      ...
      ElMessage.error('取消订单失败，请检查网络或稍后再试。'); // <-- 这个用于网络或HTTP错误
    } finally {
  ...
  watch(
    () => [
      orderFilterPagination.value.page,
      orderFilterPagination.value.pageSize,
      orderFilterPagination.value.orderStatus,
    ],
    () => {
      queryOrders();
    },
	...
	// 手动触发一次订单查询，确保页面加载后立即加载数据
	// onMounted 已经调用了 queryOrders，watch 的初次执行也会触发，这里不需要额外调用
</script>
