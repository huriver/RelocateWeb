    ...
    passwordFormRef.value.validate(async (valid) => {
      if (valid) {
        ...
        try {
          ...
          // changePasswordApi 应该返回 axios 的响应对象，其 data 属性包含后端返回的 { code, msg, data }
          ...
          if (res.data.code === 1) {
            ...
            store.clearFrontSession();
            router.push({ name: 'userLogin' });
            console.log('已执行清除 store 和跳转');
          } else {
            // 后端返回业务错误 (code !== 1)，request.js 拦截器已经弹窗提示了 res.data.msg
            console.log('后端返回非成功 code，request.js 拦截器已处理提示');
            // ElMessage.error(res.data.msg || '密码修改失败'); // <-- 移除此行，避免重复提示
            // 这里无需额外处理，让 request.js 的拦截器统一弹窗
          }
        } catch (error) {
          ...
          ElMessage.error('密码修改失败，请稍后重试！'); // <-- 这个用于网络或HTTP错误，或 request.js 明确拒绝的情况
        } finally {
