	...
	const queryNews = async () => {
		try {
			...
			if (res.code === 1) {
				...
				console.error('获取新闻数据业务失败:', res.msg || '未知错误'); // 可以保留日志
				// ElMessage.error(res.msg || '获取新闻数据失败'); // <-- 移除此行，避免重复提示
				newsData.value = [];
				newsForm.value.total = 0;
			}
		} catch (error) {
			// 捕获真正的请求错误 (网络问题、HTTP错误等，不包括 code !== 1 的业务错误)
			console.error('网络错误或请求失败，获取新闻数据失败:', error);
			ElMessage.error('获取新闻数据失败，请稍后再试'); // <-- 这个用于网络或HTTP错误
			newsData.value = [];
			newsForm.value.total = 0;
		}
	};
...
<template>
	<div class="news-container">
		...
		<div class="pagination-footer">
			<el-pagination
				v-model:current-page="newsForm.page"
				v-model:page-size="newsForm.pageSize"
				:page-sizes="[10, 20, 30, 40]"
				layout="total, sizes, prev, pager, next, jumper"
				:total="newsForm.total"
				@size-change="queryNews"
				@current-change="queryNews"
			/>
		</div>
	</div>
</template>
...
<style scoped lang="less">
	...
	.data-list-box {
		...
		.data-item {
			...
			.title {
				color: var(--el-color-primary, rgb(64, 158, 255)); // 使用 Element Plus 主色变量
				font-size: 20px;
				font-weight: bold;
				margin-bottom: 8px;
			}
